...
function checkNoBlanks(fObj, fname)
{
	var space = " ";
 	if (fObj.value.indexOf(space) >= 0 )
			return getErrorMsgByVar("gsm_msg_space") + " %s\n";
	else return "";
}
...
function checkBW(text_input_field, field_name, min_value, bwunit, required)
{
	var str = text_input_field.value;
	var error_msg= "";
	var int_part;
	...
	else // not blank, check contents
	{
		...
		{
			int_part = str.split(".")[0];
			if(bwunit == "Mbps" && int_part.length > 3)
				error_msg = addstr("%s " + getErrorMsgByVar("gsm_msg_check_invalid"),field_name);
		}
	}
	if (error_msg.length > 1)
...
function isValidRangeField(startobj,stopobj){
	...
	var start, stop;
	var msg = checkInt(startobj, getErrorMsgByVar("gsm_msg_sev_start_port"), 1, 65535, true);
	msg += checkInt(stopobj, getErrorMsgByVar("gsm_msg_sev_finish_port"), 1, 65535, true);
	if (msg.length > 1)	{
		alert(msg);
		startobj.focus();
		return false;
	...
	if(str_stop != ""){
		if(start< 0 || start > stop || start > 65535){
			alert(getErrorMsgByVar("gsm_msg_pub_port_range"));
			return false;
		}
	}
...
// Dont know where to get convert() function, use fake one. -- Argon
