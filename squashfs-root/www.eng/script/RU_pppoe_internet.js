...
*/
	...
	{
		location.href = "BRS_RUS_manual_pppoe_account.html";	
	}
...
function checkStaticIP()
{
	...
	var dns_choices_div = document.getElementById("staticDNS_choices_div");
	var dns_choices = dns_choices_div.getElementsByTagName("input");
	var forms = document.getElementsByTagName("form");
	var cf = forms[0];
/*
    if(ip_choices[1].checked)
    {
		if(check_static_ip()==false)
			return false;
		if(check_ether_samesubnet()==false)
			return false;
	}
	if(dns_choices[1].checked)
    {
...
function check_static_ip()
{
	...
	if(checkipaddr(ether_ipaddr.value)==false )
	{
