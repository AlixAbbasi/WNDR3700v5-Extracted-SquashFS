...
//        connectionID_div.appendChild(connectionID_text);
	...
	var Subnet_Mask_input = document.getElementById("inputNetMask");
	Subnet_Mask_input.onkeypress = ipaddrKeyCode;
...
function checkPPTP()
{
	...
	if(!checkIPaddr())
		return false;
	//document.getElementById("pptp_passwd").value = pptp_passwd.value;
	document.getElementById("c4_myip_").value = IP_addr.value;
	document.getElementById("c4_mygw_").value = gateway.value;
	cf.submit();
...
function checkIPaddr()
{
	...
	var pptp_mask = document.getElementById("inputNetMask");
	...
	if(pptp_myip.value != "")
	{
		...
		if(pptp_mask.value != "" && checksubnet(pptp_mask.value)==false)
		{
