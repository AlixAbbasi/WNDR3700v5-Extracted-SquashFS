...
function replaceLanguage(){
    ...
    for(var i=0; i<all_tag.length; i++){
        ...
        if(tempLanAtt){
            ...
            if(msg != null && msg != ""){
/*In Genie GUI, have a new tag <U>, so multi_lan js should be modify together
*If add other new Tag, here should be modify too.
*/
            	if(tagName == 'u')
...
function apply_disable()
{
	...
	var form_size = cf.elements.length;
       ...
       {
        if (cf.elements[i].type.toLowerCase()=="submit" 
        ||cf.elements[i].name.toLowerCase()=="apply"
        )
        {
        
            cf.elements[i].disabled = true;
            cf.elements[i].className="common_gray_bt";
        }//else if(cf.elements[i].name.toLowerCase()=="apply" || cf.elements[i].name.toLowerCase()=="cancel" || cf.elements[i].value.toLowerCase()=="cancel")
        			//cf.elements[i].disabled = true;
        }
      }
