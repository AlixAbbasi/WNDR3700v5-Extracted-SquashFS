...
function setDNS(cf)
{
	var dflag = cf.dyn_get_dns.checked;
	if(cf.dyn_get_ip.checked == false)
	{
		cf.dyn_get_dns.checked = false;
		dflag = false;
...
function check_static_ip_mask_gtw()
{
	...
	if(checksubnet(form.hid_ap_subnet.value)==false)
	{
...
function checkadv(form)
{
	...
	{	
		alert("CTS/RTS Threshold value is outside the range 1-2347.");
		return false;
	}
	...
	{
		alert("Fragmentation Length value is outside the range 256-2346.");
		return false;
	}
	...
	if(form.enable_apmode.checked == true)
	{
		...
		if(form.dyn_get_ip.checked == true)
		...
		if(form.dyn_get_dns.checked == true)
			form.hid_dyn_get_dns.value="1";
		else
		{
			if(check_static_dns(!(form.dyn_get_ip.checked)) == false)
				 return false;
			form.hid_dyn_get_dns.value="0";
		}
	}
	else
	{
		form.hid_enable_apmode.value="0";		
		if(form.dyn_get_ip.checked == true)
			form.hid_dyn_get_ip.value="1";
		else
			form.hid_dyn_get_ip.value="0";
...
/*
	if( an_router_flag == 1){
		...
		if( old_endis_wl_radio == "1" && form.wl_enable_router.value == "0" && old_endis_wla_radio == "1" && form.wla_enable_router.value == "0" ){
			if(!confirm("WPS is going to become inaccessible. Do you want to continue?"))
				return false;
		}
		else if( old_endis_wl_radio == "1" && form.wl_enable_router.value == "0" )
		{
			if(!confirm("WPS in b/g/n band is going to become inaccessible. Do you want to continue?"))
				return false;
		}
		else if( old_endis_wla_radio == "1" && form.wla_enable_router.value == "0" )
		{
			if(!confirm("WPS in a/n band is going to become inaccessible. Do you want to continue?"))
				return false;
		}
	}
	else{
...
*/
	...
	else if( old_endis_wl_radio =="0" && form.wl_enable_router.value == "1" )
	{
		if ( old_wds_endis_fun == "1" &&  old_wds_repeater_basic == "0" )
		{
			...
			{
				if(!confirm("The wireless security options WPA-PSK (TKIP), WPA2-PSK (AES), and WPA-PSK (TKIP) + WPA-PSK (AES) are not available if you enable the wireless repeating function. \nPlease change your security option before you enable the wireless repeating function."))
					return false;
				else
				{
