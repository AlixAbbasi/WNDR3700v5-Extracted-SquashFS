...
var _dev_name=3;                                                                    /* MAC        */
var TYPE_APP=1, TYPE_GAME=2, TYPE_PORT=-1, TYPE_MAC=3, TYPE_NEW_APP=11, TYPE_NEW_GAME=21;
...
var CATEGORY_APP=0, CATEGORY_GAME=1, CATEGORY_PORT=-1, CATEGORY_MAC=2;
	...
	for(i=0; i<apply_qos.length; i++) {
	    ...
	    if (i != exceptIdx){ //do not compare with itself...common sense
			if (mac.toUpperCase() == rule[_src].toUpperCase()){
				alert("#"+index+":"+mac+"-"+getErrorMsgByVar("gsm_msg_qos_mac_conflicts"));
				return true;
			}
		}
		index++;
...
function parse_apply_qos_adv_rules() {
	 if(validRule(apply_qos_adv_rules)){
        apply_adv_qos = apply_qos_adv_rules.split("\x01");
        if(parseQoSRule(apply_adv_qos) ){
...
function show_customer() {
    ...
    if (index == cf.apps.options.length-1) {
        next_file=cf.category.value == 0 ? "QOS_add_app_new.htm" : "QOS_add_game_new.htm";
    }
    else {
        next_file=cf.category.value == 0 ? "QOS_add.htm" : "QOS_add_game.htm";
    }
    if(cf.this_file.value != next_file) {
...
function new_show_customer() {
	var cf = document.forms[0];
    var index = cf.apps.options.selectedIndex;
    var next_file;

    if (index == cf.apps.options.length-1) {
        next_file= "QOS_add_app_new.htm";
    }
    else {
        next_file="QOS_main.htm" ;
    }
    if(cf.this_file.value != next_file) {
        location.href = next_file;
    }
...
function show_customer_edit() {
    ...
    if (index == cf.apps.options.length-1) {
        next_file=cf.category.value == 0 ? "QOS_edit_app_new.htm" : "QOS_edit_game_new.htm";
    }
    else {
        next_file=cf.category.value == 0 ? "QOS_edit.htm" : "QOS_edit_game.htm";
    }
    if(cf.this_file.value != next_file) {
        cf.todo.value="qos_edit";
        cf.next_file.value = next_file;
...
function setPolicyName()
{
    ...
    if (cf.category.value == 0) {
    	index = cf.apps.options.selectedIndex;
        if (index != cf.apps.options.length-1) {
 //           cf.name.value = cf.apps[cf.apps.selectedIndex].text;
			cf.name.value = predef_qos[cf.apps.selectedIndex][_name];
//            cf.name.disabled=true;
    ...
    else if (cf.category.value == 1) {
    	index = cf.apps.options.selectedIndex;
    	if (index != cf.apps.options.length-1) {
 //           cf.name.value = cf.apps[cf.apps.selectedIndex].text;
			cf.name.value = predef_qos[cf.apps.selectedIndex + 20][_name];
//            cf.name.disabled=true;
        }
        else {
            cf.name.value = "";
...
function new_setPolicyName()
{
	parse_predef_qos_rules();
	var cf = document.forms[0];
	var index;
  index = cf.apps.options.selectedIndex;
  if(index != cf.apps.length - 1)
  {
  	cf.priority.selectedIndex = predef_qos[index][_prio];
  }
}

function show_apply_rules()
{
	var highest_list = "<ul type=none>";
	var high_list = "<ul type=none>";
	var normal_list = "<ul type=none>";
	var low_list = "<ul type=none>";
	var i;
	...
	for(i = 0; i < apply_qos.length; i ++)
	{
		if(apply_qos[i][_prio] == 0)
		{
			highest_list = highest_list + "<li>" + "<span onmousemove=\"changecursor(this)\" onmouseout=\"resumecursor(this)\" onclick=\"clickit(this)\">" + apply_qos[i][_name] + "</span>" + "</li>";
		}
		
		if(apply_qos[i][_prio] == 1)
		{
			high_list = high_list + "<li>" + "<span onmousemove=\"changecursor(this)\" onmouseout=\"resumecursor(this)\" onclick=\"clickit(this)\">" + apply_qos[i][_name] + "</span>" + "</li>";
		}
		
		if(apply_qos[i][_prio] == 2)
		{
			normal_list = normal_list + "<li>" + "<span onmousemove=\"changecursor(this)\" onmouseout=\"resumecursor(this)\" onclick=\"clickit(this)\">" + apply_qos[i][_name] + "</span>" + "</li>";
		}
		
		if(apply_qos[i][_prio] == 3)
		{
			low_list = low_list + "<li>" + "<span onmousemove=\"changecursor(this)\" onmouseout=\"resumecursor(this)\" onclick=\"clickit(this)\">" + apply_qos[i][_name] + "</span>" + "</li>";
		}
	}
	
	highest_list = highest_list + "</ul>";
	high_list = high_list + "</ul>";
	normal_list = normal_list + "</ul>";
	low_list = low_list + "</ul>";
	document.getElementById("highest").innerHTML = highest_list;
	document.getElementById("high").innerHTML = high_list;
	document.getElementById("normal").innerHTML = normal_list;
	document.getElementById("low").innerHTML = low_list;
}
/*
...
function setPolicyItems() {
    ...
    switch(parseInt(cf.category.value, 10)) {
    case CATEGORY_APP:
    case CATEGORY_GAME:
        ...
        for(n=0; n<cf.apps.options.length; n++) {
            if(apply_qos[i][apply_qos[i].length==5?_name:_desc_name] == cf.apps.options[n].id) {
                index = n;
                break;
            }
        }
...
function parseOnePortRule(rule){
    ...
    //------------SOURCE     PORT-----------------------
	if(rule[_src] != "L7") //skip layer7 rules
	{
...
function ruleConflict(rule, ingore_idx){
	...
	/* skip check when using L7 rules */
	if(rule[_src] == "L7")
		return result;
    ...
    case CATEGORY_GAME:
        ...
        for(i=0; i<apply_qos.length; i++) {
			...
			/* Do not compare with L7 rule */
			if(apply_qos[i][_src] == "L7")
			{
				continue;
			}
...
function new_set_str_rulePriority(str_rule, str_prio){
	var cf = document.forms[0];
    var ruleArr = str_rule.split("\x02");
    ruleArr[_prio] = str_prio;
    return (parts_arrayToString(ruleArr));
...
function policy_name_is_predefined() {
    ...
    if(cf.category.selectedIndex == CATEGORY_APP) {
...
//            && cf.name.value == cf.apps.options[cf.apps.selectedIndex].text) {
    ...
    if(cf.category.selectedIndex == CATEGORY_GAME) {
        if (cf.apps.options.selectedIndex != cf.apps.options.length-1
//            && cf.name.value == cf.apps.options[cf.apps.selectedIndex].text) {
			  && cf.name.value == predef_qos[cf.apps.selectedIndex + 20][_name]) {
            /* User selected predefined rules no need to check */
...
function new_policy_name_is_exist()
{
	  var cf = document.forms[0];
    var i;
    for(i=0; i<apply_qos.length; i++){
        if(cf.policy_name.value == apply_qos[i][_name]) {
            return true;
        }
    }
    return false;
...
function click_apply(){
	...
	if(!ed_mode_flag && currentChosenType == TYPE_APP)
	{
		for(i = 0; i < apply_qos.length; i ++)
		{
			if(cf.apps.options[cf.apps.selectedIndex].text == apply_qos[i][_name] && apply_qos[i][_src] == "L7")
			{
       		 	alert(getErrorMsgByVar("gsm_msg_qos_conflict_predef_name"));
				return false;
			}
	...
	switch(currentChosenType) {
		case TYPE_GAME:
			apps_index = cf.apps.selectedIndex + 20;
		case TYPE_APP:
			str_new_rule = get_predefined_rules(predef_qos[apps_index][_name]); //copy from rule template
        ...
        case TYPE_NEW_GAME:
            ...
            */
            ...
            proto = parseInt(cf.protocol.value, 10);
            name = cf.name.value;
            srcpart = "+"; //reserved
...
function new_add_rules(){
    var cf = document.forms[0];
    var result="";
    var n=0;
    var name="";
    var array_new_rule="", str_new_rule="";
    var currentChosenType; // "1" is by app, "2" is by device
    var predefined_rule_list;
    var qos_edit_idx = -1;
    var dev_name;
    var mac;
    ...
    if(currentChosenType == "1") {
        apps_index = cf.apps.selectedIndex;       
    
		for(i = 0; i < apply_qos.length; i ++)
		{
			if(cf.apps.options[cf.apps.selectedIndex].text == apply_qos[i][_name])
			{
       		 	alert(getErrorMsgByVar("gsm_msg_qos_conflict_predef_name"));
				return false;
			}
    ...
    /* Build str_new_rule for app&game rules at first */
			...
			str_new_rule = new_set_str_rulePriority(str_new_rule, cf.priority.selectedIndex);
    ...
    if(currentChosenType == "2")
		{
			 doing_add_mac = true;
       name = cf.policy_name.value;
       dev_name = cf.dev_name.value;
       mac = cf.mac.value;
       prio = cf.mac_priority.selectedIndex;
       str_new_rule = makeRule(CATEGORY_MAC, name, mac, dev_name, prio);
       array_new_rule = parseOneMacRule(str_new_rule);
       if(array_new_rule!="err"){
       	if (ruleConflict(array_new_rule, qos_edit_idx)) {   		
          ...
          result = addRule(apply_qos_rules, str_new_rule);
       }
       else {
          return false;
    	 }
    ...
    cf.apply_qos_rules.value = result;
    apply_qos_rules = result;   
    return true;
}

function get_rule_idx_byname(rulename)
{
	var i;
	for(i = 0; i < apply_qos.length; i ++)
	{
		if(apply_qos[i][_name] == rulename)
		{
			break;
		}
	}
