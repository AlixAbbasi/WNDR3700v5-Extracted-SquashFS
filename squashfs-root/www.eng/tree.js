...
function xbDetectBrowser()
{
	...
	else if ((ua.indexOf('mozilla') !=-1) && (ua.indexOf('spoofer')==-1) && (ua.indexOf('compatible') == -1) && (ua.indexOf('opera')==-1)&& (ua.indexOf('webtv')==-1) && (ua.indexOf('hotjava')==-1))
	{
		...
		navigator.org = 'netscape';
		navigator.family = 'nn' + parseInt(navigator.appVersion, 10);
	}
...
// *****************************************************************
function Folder(folderDescription, hreference, deviceDescription) //constructor
{
	//constant data
	...
	this.iconSrc = ICONPATH + "treefolderopen.gif";
  	this.iconSrc_Rbase = ICONPATH + "rout.gif";
	this.iconSrc_base = ICONPATH + "usb.gif";
...
function initializeFolder(level, lastNode, leftSide)
{
	...
	if (browserVersion == 0 || STARTALLOPEN==1)	this.isOpen=true;
...
function drawFolder(insertAtObj)
{
	...
	if (this.level>0)
		if (this.isLastNode) //the last child in the children array
			...
			leftSide = leftSide + "<td valign=top background=" + ICONPATH + "treevertline.gif>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>";
	...
	{
		if (this.prependHTML == "")
			docW = docW + "<img src=" + ICONPATH + "treeblank.gif height=2 width=2>";
	}
	if (WRAPTEXT)
...
function linkFolderHTML(isTextLink)
{
	...
	if (this.hreference)
	{
		...
		if (isTextLink) {
			docW += "class='TR_Node' id=\"itemTextLink"+this.id+"\" ";
		}
...
function nodeImageSrc()
{
	...
	{
		if (this.nChildren == 0)
			srcStr = ICONPATH + "treelastnode.gif";
		else
			if (this.isOpen)
				srcStr = ICONPATH + "treemlastnode.gif";
			else
				srcStr = ICONPATH + "treeplastnode.gif";
	}
	else
	{
		if (this.nChildren == 0)
			srcStr = ICONPATH + "treenode.gif";
		else
			if (this.isOpen)
				srcStr = ICONPATH + "treemnode.gif";
			else
				srcStr = ICONPATH + "treepnode.gif";
	}
	return srcStr;
}
...
function iconImageSrc()
{
  ...
  {
	if (this.device == "ROUT")
	{ return(this.iconSrc_Rbase) }
	else if (this.device == "USB")
	{ return(this.iconSrc_base) }
	else
	{ return(this.iconSrc) }
  }
  else
...
function drawItem(insertAtObj)
{
	...
	*/
	...
	if (this.level>0)
		...
		{
			leftSide = leftSide + "<td valign=top><img src='" + ICONPATH + "treelastnode.gif' width=16 height=22></td>";
		}
		else
		{
			leftSide = leftSide + "<td valign=top background=" + ICONPATH + "treevertline.gif><img src='" + ICONPATH + "treenode.gif' width=16 height=22></td>";
		}
	...
	if (USEICONS)
		...
		if (this.prependHTML == "")
			docW = docW + "<img src=" + ICONPATH + "treeblank.gif height=2 width=3>";
	...
	if (USETEXTLINKS)
		docW = docW + "<a class='TR_Node' " + fullLink + " id=\"itemTextLink"+this.id+"\">" + this.desc + "</a>";
	else
		docW = docW + this.desc;
...
function leftSideHTML(leftSideCoded)
{
	...
	for (i=0; i<leftSideCoded.length; i++)
	{
		...
		{
			retStr = retStr + "<td valign=top background=" + ICONPATH + "treevertline.gif><img src='" + ICONPATH + "treevertline.gif' width=16 height=22></td>";
		}
		if (leftSideCoded.charAt(i) == "0")
		{
			retStr = retStr + "<td valign=top><img src='" + ICONPATH + "treeblank.gif' width=16 height=22></td>";
		}
	}
	return retStr;
}
...
{
	//random_num is added by joanw for the problem of IE won't really reload the page.
	var d=new Date();
	if(target.indexOf("?")==-1)	target+="?";
	else				target+="&";
	target+="random_num="+d.getMonth()+d.getDate()+d.getFullYear()+d.getHours()+d.getMinutes()+d.getSeconds();
...
function gFld(description, hreference, deviceDesc)
{
	folder = new Folder(description, hreference, deviceDesc);
	return folder;
}
...
function oldGLnk(target, description, linkData)
{
	...
	else
	{
		...
		{
			if (target==1)	fullLink = "'http://"+linkData+"' target=_self";
			else		fullLink = "'http://"+linkData+"' target=_self";
		}
	}
...
function getElById(idVal)
{
	...
	alert(getErrorMsgByVar("gsm_msg_tree_err_geteleid"));
	return null;
}
			...
			if (fldStr != "") {
				...
				if (nodeObj!=null) //may have been deleted
					if (nodeObj.setState) {
						...
						alert(getErrorMsgByVar("gsm_msg_tree_inv_id_use_ext_id"));
			}
		}
	}
}
...
function handleResize(evt)
{
	...
	{
		folderNum = indexOfEntries[i];
		propagateChangesInState(folderNum);
	}
}

// IP Address
var ipadd = self.location.host;
function dw(message)
