...
function checkPort(name, port)
{
	...
	{
		msg = "Invalid " + name + ", must be in the range 1-65535.\n";
		alert(msg);
		return false;
	}
	return true;
...
function check_triggering_add(cf,flag)
{
	...
	if(checkPort("Triggering Port", cf.pt_port.value)==false)
		return false;
...
/*	if(cf.pt_port.value == "21" )
	...
	if(checkPort("Starting Port", cf.in_port_start.value)==false)
		return false;
	cf.in_port_start.value=parseInt(cf.in_port_start.value,10);

	if (cf.in_port_end.value.length > 0)
	{
		if(checkPort("Ending Port", cf.in_port_end.value)==false)
			return false;
		...
		{//bug 23479:use parseInt to parsing string
			alert("Invalid inbound connection port range.\n");
			return false;
		}
	}
	if (cf.in_port_end.value.length == 0)
...
/*	if( all_port_num > 256)
	{
		alert("The total number of the incoming port(s) should be less than 256!");
		return false;
	} */
...
function check_triggering_edit(cf)
{
	...
	location.href="triggering_edit.htm";
	return false;
}
...
function check_triggering_apply(cf)
{
	...
	{
		alert("Invalid Port triggering timeout, please enter proper numeral.\n");
		return false;
	}
