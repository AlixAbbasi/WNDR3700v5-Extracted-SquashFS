...
function getSelIndex(sel_object, sel_text)
{
	...
	if (DEBUG)
		alert(getErrorMsgByVar("gsm_msg_pub_debug") + sel_object.name + getErrorMsgByVar("gsm_msg_pub_inv_sel") + sel_text + msg_pub_1_instead);
...
function getRadioIndex(radio_object, checked_value)  // find index matching checkecd_value, 0 if no match
{
	...
	if (DEBUG)
		alert(getErrorMsgByVar("gsm_msg_pub_debug") + radio_object.name + getErrorMsgByVar("gsm_msg_pub_inv_sel") + checked_value + getErrorMsgByVar("gsm_msg_pub_1st_instead"));
	return  0;   // if no match
}
