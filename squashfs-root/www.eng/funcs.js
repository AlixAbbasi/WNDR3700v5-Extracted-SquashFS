...
//IE 11 has no MSIE mark in userAgent! So we have to find another way.
//But this code is from Internet and haven't been tested thoroughly so may not be correct!
//Be careful if want to use it. 
function isIE11(){
...
function isIE9above(){
	if(navigator.userAgent.indexOf("MSIE") != -1){
		var tmp_start_index = navigator.userAgent.indexOf("MSIE");
		tmp_start_index = tmp_start_index+5;
		var tmp_str_ary = navigator.userAgent.slice(tmp_start_index, "-1").split(';');
		
		return (parseInt(tmp_str_ary[0],10) >= 9);
	}else{
		return false;
	}
}
