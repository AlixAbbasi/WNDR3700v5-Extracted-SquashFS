var invalid_port_used="The specified port(s) are being used by other configurations. Please check your configurations of USB Readyshare, Remote Management, Port forwarding, Port Triggering, UPnP Port Mapping table, RIP, and Internet connection type";
...
function check_list_length(forward_num)
{
	...
	{	
		if(forward_num==20)
		{
			alert("The router can support only 20 port forwarding rules.");
			return false;
		}
	}
	else
  ...
  cf.internal_port_end.value = parseInt(cf.internal_port_start.value)+(parseInt(cf.port_end.value)-parseInt(cf.port_start.value));
	...
	for(i=1;i<=forward_array_num;i++)
	{
		...
		if(flag == 'edit')
		{
			...
			{
				alert("Service name already in use, and cannot be used twice.");
				return false;
			}
		}
		else
	...
	for(i=1;i<=forward_array_num;i++)
	{
		...
		else if (sertype=="TCP/UDP"||sertype==input_sertype||input_sertype=="TCP/UDP")
		{
			if(flag == 'edit')
			{
				if (((!(parseInt(ext_endport)<parseInt(input_ext_start_port)||parseInt(input_ext_end_port)<parseInt(ext_startport))) && select_editnum!=i) || ((!(parseInt(int_endport)<parseInt(input_int_start_port)||parseInt(input_int_end_port)<parseInt(int_startport))) && select_editnum!=i))
 				{
...
function check_forwarding_edit(cf)
{
	...
	location.href ="forwarding_edit_wait.htm";
	return false;
}
...
function Check_add(cf)     
{
	...
	for(i=1;i<=forward_array_num;i++)
	{
		...
		{
			if (!(parseInt(ext_endport)<parseInt(input_external_start_port)||parseInt(input_external_end_port)<parseInt(ext_startport)) || !(parseInt(int_endport)<parseInt(input_internal_start_port)||parseInt(input_internal_end_port)<parseInt(int_startport)))
 			{
				alert(invalid_port_used);
				return false;
			}		
