var hour_schedule_12=["12:00 midnight."/*0*/,"12:30 a.m."/*1*/,"01:00 a.m."/*2*/,"01:30 a.m."/*3*/,"02:00 a.m."/*4*/,"02:30 a.m."/*5*/,"03:00 a.m."/*6*/,"03:30 a.m."/*7*/,"04:00 a.m."/*8*/,"04:30 a.m."/*9*/,"05:00 a.m."/*10*/,"05:30 a.m."/*11*/,"06:00 a.m."/*12*/,"06:30 a.m."/*13*/,"07:00 a.m."/*14*/,"07:30 a.m."/*15*/,"08:00 a.m."/*16*/,"08:30 a.m."/*17*/,"09:00 a.m."/*18*/,"09:30 a.m."/*19*/,"10:00 a.m."/*20*/,"10:30 a.m."/*21*/,"11:00 a.m."/*22*/,"11:30 a.m."/*23*/,"12:00 noon"/*24*/,"12:30 p.m."/*25*/,"01:00 p.m."/*26*/,"01:30 p.m."/*27*/,"02:00 p.m."/*28*/,"02:30 p.m."/*29*/,"03:00 p.m."/*30*/,"03:30 p.m."/*31*/,"04:00 p.m."/*32*/,"04:30 p.m."/*33*/,"05:00 p.m."/*34*/,"05:30 p.m."/*35*/,"06:00 p.m."/*36*/,"06:30 p.m."/*37*/,"07:00 p.m."/*38*/,"07:30 p.m."/*39*/,"08:00 p.m."/*40*/,"08:30 p.m."/*41*/,"09:00 p.m."/*42*/,"09:30 p.m."/*43*/,"10:00 p.m."/*44*/,"10:30 p.m."/*45*/,"11:00 p.m."/*46*/,"11:30 p.m."/*47*/];
...
function check_schedule_onoff()
{
	...
	cf.wirless_schedule_an.disabled = disable_flag;
	cf.add_period_an.disabled = disable_flag;
	cf.edit_period_an.disabled = disable_flag;
	cf.delete_period_an.disabled = disable_flag;

	if(wireless_schedule_num_a > 1)
	{
		for(i=0; i<wireless_schedule_num_a; i++)
		{
			cf.wireless_schedule_item_a[i].disabled = disable_flag;				
		}
	}
	else if(wireless_schedule_num_a > 0)
		cf.wireless_schedule_item_a.disabled = disable_flag;
}
...
//	if(cf.wirless_schedule.checked && cf.enable_ap.checked && ntp_updated=="1")
	...
	//	setDisabled(false, cf.add_period, cf.edit_period, cf.delete_period);
	}	
	else
	{	
	...
	//	setDisabled(true, cf.add_period, cf.edit_period, cf.delete_period);
	}

//	if(cf.wirless_schedule_an.checked && cf.enable_ap_an.checked && ntp_updated=="1")
    if(cf.enable_ap_an.checked && ntp_updated=="1")
	{
	//	setDisabled(false, cf.add_period_an, cf.edit_period_an, cf.delete_period_an);
		cf.wirless_schedule_an.disabled = false;
	...
	{
	//	setDisabled(true, cf.add_period_an, cf.edit_period_an, cf.delete_period_an);
	cf.wirless_schedule_an.disabled = true;
...
function check_wireless_schedule(chan_type, btn_flag)
{
	...
	if(chan_type == "bgn")
	{
		if(btn_flag == "add")
		{
			...
			{
				alert("O roteador só pode aceitar 21 períodos.");
				return false;
			}
			else
				...
				else
				{
					if(cf.wireless_schedule_item.checked)
						cf.wladv_schedule_edit_num.value = cf.wireless_schedule_item.value;
                                ...
                                {
//                                      alert("Selecione um item para edição.");
				...
				else
				{
					if(cf.wireless_schedule_item.checked)
						cf.wladv_schedule_delete_num.value = cf.wireless_schedule_item.value;
                                ...
                                {
//                                      alert("Selecione um item para exclusão.");
		...
		else if(btn_flag == "edit")
		{
			if(wireless_schedule_num_a > 0)
			{
				...
				if(wireless_schedule_num_a > 1)
				{
					for(i=0; i<wireless_schedule_num_a; i++)
					{
						if(cf.wireless_schedule_item_a[i].checked)
							cf.wladv_schedule_edit_num.value = cf.wireless_schedule_item_a[i].value;
					}
				}
				else
				{
					if(cf.wireless_schedule_item_a.checked)
						cf.wladv_schedule_edit_num.value = cf.wireless_schedule_item_a.value;
				}
		...
		else if(btn_flag == "delete")
		{
			...
			{	
				...
				if(wireless_schedule_num_a > 1)
				{
					...
					{
						if(cf.wireless_schedule_item_a[i].checked)
							cf.wladv_schedule_delete_num.value = cf.wireless_schedule_item_a[i].value;
					}
				}
				else
				{
					if(cf.wireless_schedule_item_a.checked)
						cf.wladv_schedule_delete_num.value = cf.wireless_schedule_item_a.value;
				}
	...
	if(array_num > 0)
	{
		for(i=1; i<=array_num; i++)
		{
			...
			if(array_value == item_value)
			{
				if(modify_flag == "add")
				{
					alert("Não é possível adicionar a mesma regras duas vezes.");
					return false;
				}
...
function wireless_schedule_add(wireless_type, modify_flag)
{
	...
	cf.action = "/apply.cgi?/WLG_schedule_close.htm timestamp="+ts;
	return true;
}
